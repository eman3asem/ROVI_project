...
def program(d, m):
    ...
    cylinder_frame = get_mjobj_frame(model=m, data=d, obj_name="cylinder") * sm.SE3.Rx(np.pi/2)* sm.SE3.Rz(np.pi/2) # Get body frame
    # for side pick: *sm.SE3.Rx(-np.pi/2)
    drop_point_cylinder_frame = get_mjobj_frame(model=m, data=d, obj_name="drop_point_box") * sm.SE3.Rx(np.pi/2)* sm.SE3.Rz(np.pi/2) # Get body frame
