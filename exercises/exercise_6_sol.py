...
def program(d, m):
    ...
    obj_frame = get_mjobj_frame(model=m, data=d, obj_name="drop_point_cylinder")*sm.SE3.Rx(-np.pi) # Get body frame
    ...
    start_q = goal_q
    # Define grasping frames for object: box
    obj_frame = get_mjobj_frame(model=m, data=d, obj_name="pickup_point_cylinder")* sm.SE3.Rx(-np.pi) # Get body frame
    goal_q = robot.robot_ur5.ik_LM(Tep=obj_frame, q0=start_q)[0]
    sol_traj = plan(d=d, m=m, start_q=start_q, goal_q=goal_q)
